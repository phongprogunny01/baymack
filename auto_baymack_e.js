	...
	if(c){
		...
		var interval = setInterval(function(){
			...
			if(window.location.href=="https://www.baymack.com/"){
				...
				//}
				if($("a[href='/challenge']:eq(0) span:eq(0)").html()!="Get 7 entries"){
					$("title").html("Close");
				} else {
...
function check(id,api){
	setTimeout(function(){
		$.get( "https://azcaptcha.com/res.php?key="+api+"&action=get&json=1&id="+id, function(data) {
			console.log(data);
			if (JSON.parse(data).request=="CAPCHA_NOT_READY"){
				...
				$("#loading").remove();
			} else if(JSON.parse(data).request=="ERROR_CAPTCHA_UNSOLVABLE"||JSON.parse(data).request=="ERROR_NO_SLOT_AVAILABLE"){
				res(api);
			}
...
function res(api){
	...
	if(api!=undefined&&api!=""){
		...
		$.get( "https://azcaptcha.com/in.php?key="+api+"&method=userrecaptcha&googlekey="+sitekey+"&json=1&pageurl=https://www.baymack.com/vids", function(data) {
			console.log(data);
			if(JSON.parse(data).status==1){
				check(JSON.parse(data).request,api);
			} else if(JSON.parse(data).request=="ERROR_RECAPTCHA_THREAD_LIMIT"){
				setTimeout(function(){
					res(api);
				},20000);
			}
