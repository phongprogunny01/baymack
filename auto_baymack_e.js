...
function check(id,api,server){
	setTimeout(function(){
		$.get( server + "res.php?key="+api+"&action=get&json=1&id="+id, function(data) {
			console.log(data);
...
function res(api,server){
	...
	if(api!=undefined&&api!=""){
		...
		$.get( server+"in.php?key="+api+"&method=userrecaptcha&googlekey="+sitekey+"&json=1&pageurl=https://www.baymack.com/vids", function(data) {
			console.log(data);
