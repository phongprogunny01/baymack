...
function check(id,api){
	...
	setTimeout(function(){
		$.get( "http://api.solvecaptcha.com/res.php?key="+api+"&action=get&id="+id, function( data,status ) {
			console.log(data);
			console.log(status);
			if(data.substring(0, 2)=="OK"){
				...
				$("textarea").html(token);
					...
					script.textContent = 'var token = "'+token+'";'+'var sitekey ="'+sitekey+'";'+'var i = 50;var ngat = 0;for(i;i>=0;i--){if(___grecaptcha_cfg.clients[i]!=undefined&&ngat==0){for (var key1 in ___grecaptcha_cfg.clients[i]) {for (var key2 in ___grecaptcha_cfg.clients[i][key1]) {if(___grecaptcha_cfg.clients[i][key1][key2].sitekey==sitekey){___grecaptcha_cfg.clients[i][key1][key2].callback(token);console.log(key1+" "+key2);}}}ngat = 1;}}';
...
function res(api){
	...
	if(api!=undefined&&api!=""){
		sitekey = $("re-captcha").attr("sitekey");
		$.get( "http://api.solvecaptcha.com/in.php?key="+api+"&method=userrecaptcha&googlekey="+sitekey+"&pageurl=https://www.baymack.com/vids", function( data,status ) {
			if(data.substring(0, 2)=="OK"){
				id = data.substring(3);
				check(id,api);
			} else if(data=="ERROR_WRONG_USER_KEY"||data=="ERROR_WRONG_USER_KEY"){
				alert("WRONG API");
			} else if(data=="ERROR_ZERO_BALANCE"){
				alert("BALANCE = 0$");
			} else if(data=="ERROR_NO_SLOT_AVAILABLE"||data=="MAX_USER_TURN"){
				setTimeout(function(){
					res(api);
				},10000);
			} else {
