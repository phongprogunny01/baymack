...
//--------------------------------Main Page------------------------------------------------------
			if(window.location.href=="https://www.baymack.com/"){
				if($("i[title='Profile']").is(":visible")){
					...
					window.location = "https://www.baymack.com/history";
				}
//--------------------------------View video----------------------------------------------------
...
//------------------------------------Start vids--------------------------------------------------------------
	...
	localStorage.setItem("api","59cba6fb9db0b9b55923945149574afd");
...
//---------------------------------------------------------------------------------------------------------				
	...
	if(document.getElementsByTagName("title")[0].innerText!="Just a moment..."){
		$.get("header/info",function(data){
			if(data.user.entries < 6){
				$("title").html(data.user.entries+"/6 entries");
				wcodebyttp(1);
			} else {
				$("title").html("Close");
				$("body").remove();
...
//--------------------------------History-------------------------------------------------------
				...
				var check_show_history =  setInterval(function(){
					if($("div[class='historyblock'] div h2 a").is(":visible")){
						...
						if(parseFloat($("div[class='historyblock'] div h2 a").html().substring(1))<0.01){
